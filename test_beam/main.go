...
func main() {
	...
	lines := textio.Read(scope, "../read_file/data.txt")
	formatted := beam.ParDo(scope, format_line, lines)
	beam.ParDo(scope, fmt.Println, formatted)

}

func format_line(line string) string {
	return fmt.Sprintf("line: %s", line)
}

package main

import (
	"bufio"
	"encoding/json"
	"fmt"
	"log"
	"os"
	"strconv"
	"strings"
)

type FieldFormat struct {
	Offset int
	Length int
}

type Format struct {
	LgRecord int
	Fields   map[string]*FieldFormat
}

func main() {
	format := read_format()
	line_channel := make(chan string)
	result_channel := make(chan map[string]string)
	end_channel := make(chan bool)
	end_worker_channel := make(chan bool)
	end_printer_channel := make(chan bool)

	nb_workers := 3

	go func() {
		nb_workers_stopped := 0
		for nb_workers_stopped != nb_workers {
			select {
			case _ = <-end_worker_channel:
				nb_workers_stopped++
			case result := <-result_channel:
				res := print_result(result)
				fmt.Println(res)
			}
		}
		end_printer_channel <- true
	}()

	for i := 0; i < nb_workers; i++ {
		go func() {
			stop := false
			for !stop {
				select {
				case _ = <-end_channel:
					stop = true
					end_worker_channel <- true
				case line := <-line_channel:
					result := parse_line(format, line)
					result_channel <- result
				}
			}
		}()
	}

	read_file(format, line_channel, end_channel, nb_workers)

	_ = <-end_printer_channel

	fmt.Println("end main")
}

func print_result(result map[string]string) string {
	res, err := json.Marshal(&result)

	if err != nil {
		panic("Cannot marshal format !")
	}
	return string(res)
}

func read_file(format Format, line_channel chan string, end_channel chan bool, nb_workers int) {
	f, err := os.Open("../data2.txt")

	if err != nil {
		log.Fatal(err)
	}

	defer f.Close()

	scanner := bufio.NewScanner(f)

	for scanner.Scan() {
		line := scanner.Text()
		line_channel <- line
	}

	for i := 0; i < nb_workers; i++ {
		end_channel <- true
	}

	if err := scanner.Err(); err != nil {
		log.Fatal(err)
	}
}

func parse_line(format Format, line string) map[string]string {
	result := make(map[string]string)
	for k, v := range format.Fields {
		result[k] = line[v.Offset : v.Offset+v.Length]
	}

	return result
}

func read_format() Format {
	f, err := os.Open("../format.txt")

	if err != nil {
		log.Fatal(err)
	}

	defer f.Close()

	scanner := bufio.NewScanner(f)
	// {
	// 	field_name: {
	// 		offset: "",
	// 		length: "",
	//      value: ""
	// 	}
	// }
	field_map := make(map[string]*FieldFormat)
	temp_lgRecord := 0

	for scanner.Scan() {
		line := scanner.Text()
		field_data := strings.Split(line, " ")
		if strings.HasPrefix(line, "LGRECORD") {
			lgRecord, err := strconv.Atoi(field_data[1])
			if err != nil {
				panic(fmt.Sprintf("erreur format LgRecord: %s", field_data[1]))
			}
			temp_lgRecord = lgRecord
		} else {
			if len(field_data) != 3 {
				panic(fmt.Sprintf("length field_data != 3: %v, line: %s", field_data, line))
			}
			offset, err := strconv.Atoi(field_data[1])
			if err != nil {
				panic(fmt.Sprintf("erreur format offset: %s", field_data[1]))
			}
			length, err := strconv.Atoi(field_data[2])
			if err != nil {
				panic(fmt.Sprintf("erreur format length: %s", field_data[2]))
			}
			field_map[field_data[0]] = &FieldFormat{Offset: offset, Length: length}
		}
	}

	format := Format{LgRecord: temp_lgRecord, Fields: field_map}

	if err := scanner.Err(); err != nil {
		log.Fatal(err)
	}

	return format
}

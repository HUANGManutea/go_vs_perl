...
my $line_channel = Thread::Queue->new();
# queue des résulats après parsing, input du printer
my $result_channel = Thread::Queue->new();
...
sub main {
    ...
    # on lance le printer
    push(@threads, threads->create('run_printer'));

    # init des workers (parsers)
    for (1..$nb_workers) {
        push(@threads, threads->create('run_parser'))
    }
...
sub run_printer {
    ...
    while($nb_worker_stopped < $nb_workers) {
        my $result = $result_channel->dequeue();
        if (!defined($result)) {
            $nb_worker_stopped++;
        } else {
...
sub read_file {
    ...
    while(my $line = <$file_data_fh>) {
        ...
        if (scalar(@buffer_line) == $buffer_size) {
            ...
            $line_channel->enqueue($line_batch);
            # on reinit
            @buffer_line = ();
            $batch_id+=1;
        }
